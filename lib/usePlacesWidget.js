...
function usePlacesWidget(props) {
  ...
  (0, _react.useEffect)(function () {
    ...
    var handleAutoComplete = function handleAutoComplete(service) {
      if (apiKey && !service.maps.places || !apiKey && !google.maps.places) return console.error("Google maps places API must be loaded.");
      if (!inputRef.current instanceof HTMLInputElement) return console.error("Input ref must be HTMLInputElement.");
      autocompleteRef.current = new (service || google).maps.places.Autocomplete(inputRef.current, config);
