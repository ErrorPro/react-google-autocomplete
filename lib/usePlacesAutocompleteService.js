...
function usePlacesAutocompleteService(_ref) {
  ...
  (0, _react.useEffect)(function () {
    ...
    var buildService = function buildService(service) {
      console.log(service.maps.places, "service.maps.places");
      console.log(service.maps, "service.maps"); // eslint-disable-next-line no-undef

      placesAutocompleteService.current = new (service || google).maps.places.AutocompleteService(); // eslint-disable-next-line no-undef

      placesService.current = new (service || google).maps.places.PlacesService(document.createElement("div"));
      if (sessionToken) autocompleteSession.current = new (service || google).maps.places.AutocompleteSessionToken();
    };
